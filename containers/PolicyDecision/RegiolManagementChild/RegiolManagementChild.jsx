...
class RegiolManagementChild extends Component {
  ...
  componentDidUpdate = (prevState) => {
    const { interList, subeditDistrictInfoThing, subloadUnitNames, loadPlanLoadChild, loadPlanloadchildsr } = this.props.data
  ...
  handleDeleteRoad = (id) => {
    ...
    confirm({
      ...
      onOk() {
        that.props.getsubdeleteDistrict(id).then((res) => {
          const { code } = res.data
          if (code === 200) {
    ...
    return (
      <div id="mapContainer" className={styles.InterManagementWrapper}>
        ...
        <div className={styles.interMonitorBox} style={{ left: `${interMonitorLeft}px` }}>
          ...
          <div className={styles.title}>子区域查询</div>
          ...
          <div className={styles.OptimizingBtns}><span>子区管理</span></div>
          <div className={styles.addtask}>
            <span onClick={this.addIntersection}>添加子区</span>
          </div>
